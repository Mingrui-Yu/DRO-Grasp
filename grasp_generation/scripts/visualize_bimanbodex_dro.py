...
class ViewerApp:
    ...
    def __init__(self, server, run, hand_model, args, initial_scene):
        self.server = server
        self.run = run
        ...
        self.mode = server.gui.add_dropdown(
            "Display mode",
            options=["single_stage", "three_poses"],
    ...
    def render(self):
        ...
        for stage_name in prepared.stage_names:
            ...
            opacity = 0.48 if overlay and stage_name != prepared.selected_stage else 0.88
            self._scene_handles.append(
                self.server.scene.add_mesh_simple(
                    f"/shadow/{stage_name}",
                    mesh.vertices,
...
def main() -> None:
    ...
    ViewerApp(server, run, hand_model, args, scene_id)
